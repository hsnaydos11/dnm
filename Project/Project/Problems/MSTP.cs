...
namespace Project.Problems
{
    class Graph6
    {
        ...
        // parent[]
        public static void printMST(int[] parent, int[,] graph6)
        {
            Console.WriteLine("Edge \tWeight");
            for (int i = 1; i < V; i++)
                Console.WriteLine(parent[i] + " - " + i + "\t" + graph6[i, parent[i]]);
        }

        // Function to construct and
        // print MST for a graph represented
        // using adjacency matrix representation
        static void primMST(int[,] graph6)
        {
            ...
            for (int count = 0; count < V - 1; count++)
            {
                ...
                for (int v = 0; v < V; v++)
                    ...
                    // smaller than key[v]
                    if (graph6[u, v] != 0 && mstSet[v] == false
                        && graph6[u, v] < key[v])
                    {
                        parent[v] = u;
                        key[v] = graph6[u, v];
